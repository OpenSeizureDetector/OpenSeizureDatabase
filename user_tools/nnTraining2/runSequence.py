        ...
        if args['train']:
            ...
            if (not os.path.exists(allDataCsvPath)):
                ...
                if nestedKfold > 1:
                    ...
                    validation_prop = float(configObj.get('dataProcessing', {}).get('validationProp', 0.0) or 0.0)
                    val_csv = os.path.join(outFolder, valCsvFname)
                    require_val = (validation_prop > 0)
                    has_train_test = os.path.exists(test_csv) and os.path.exists(train_csv)
                    has_val = (not require_val) or os.path.exists(val_csv)
                    if not (has_train_test and has_val):
                        folds_present = False
            ...
            for nOuterFold in outer_folds:
                for nFold in inner_folds:
                    validation_prop = float(configObj.get('dataProcessing', {}).get('validationProp', 0.0) or 0.0)
                    use_explicit_validation = (nestedKfold <= 1 and kfold <= 1 and validation_prop > 0)
                    if nestedKfold > 1:
                    ...
                    valFoldCsvFnamePath = os.path.join(foldOutFolder, valCsvFname)
                    ...
                    if not os.path.exists(trainFoldCsvFnamePath):
                        ...
                        exit(-1)
                    if use_explicit_validation and not os.path.exists(valFoldCsvFnamePath):
                        print("ERROR: Validation CSV file %s does not exist" % valFoldCsvFnamePath)
                        print("       Set validationProp=0 to disable explicit validation split.")
                        exit(-1)
                    ...
                    valFeaturesFname = configObj['dataFileNames'].get('valFeaturesFileCsv', 'valFeatures.csv')
                    valFeaturesCsvPath = os.path.join(foldOutFolder, valFeaturesFname)
